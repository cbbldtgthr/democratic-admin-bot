...
use strum_macros::{EnumVariantNames, VariantNames};
...
use teloxide::{
    ...
    types::{
        ...
        InputMessageContentText, KeyboardButton, KeyboardMarkup, Me, ReplyMarkup,
    },
...
type MyFuture = Box<dyn Fn() -> Pin<Box<dyn Future<Output = HandlerResult> + Send>> + Send>;
...
#[derive(BotCommands, VariantNames)]
#[command(rename_rule = "lowercase")]
enum CommandBuyAndSell {
    /// Display this text
    Sell,
    /// See your listings
    MyListings,
...
impl CommandBuyAndSell {
    fn keyboard() -> KeyboardMarkup {
        KeyboardMarkup::default().resize_keyboard().append_row(
            CommandBuyAndSell::VARIANTS
                .to_vec()
                .into_iter()
                .map(|name| KeyboardButton::new(name)),
        )
    }
}
...
async fn goto_menu(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    bot.send_message(msg.chat.id, "Main menu")
        .reply_markup(keyboard(vec!["Buy-and-Sell", "Help", "Main menu"]))
        .await?;
